...
public class ShowDriversLocationActivity extends AppCompatActivity implements OnMapReadyCallback {
            ...
            if (ActivityCompat.shouldShowRequestPermissionRationale(this, REQUIRED_PERMISSIONS[0])) {
                ...
                Snackbar.make(mLayout, "이 앱을 실행하려면 위치 접근 권한이 필요합니다.",
                    ...
                    public void onClick(View view) {
                        ...
                        ActivityCompat.requestPermissions(ShowDriversLocationActivity.this, REQUIRED_PERMISSIONS, PERMISSIONS_REQUEST_CODE);
                    }
                }).show();
            } else {
    ...
    private void showDialogForLocationServiceSetting() { //checkLocationServicesStatus가 false이면 실행됨
        ...
        AlertDialog.Builder builder = new AlertDialog.Builder(ShowDriversLocationActivity.this);
                    ...
                    try {
                        if (currentPosition != null){
                            ...
                            if (resPrimary.equals("callEnd")){                                    //기사 승객 모두 boolean값이 true일 때.
                                ...
                                MarkerOptions dloc = new MarkerOptions();
    ...
    class SetMeetFlag extends AsyncTask<String, Void, String> {
        ...
        protected String doInBackground(String... strings) { //id, slocString, slocLat, slocLong, sdestLat, sdestLong 으로 6개가 필요
            try {
                ...
                //URL연결, 출력스트림 초기화
                sendMsg = "cno=" + strings[0] + "&type=" + "passenger";
                osw.write(sendMsg);
                osw.flush();
    ...
    class LocTask extends AsyncTask<String, Void, String> {
        ...
        protected String doInBackground(String... strings) { //id, slocString, slocLat, slocLong, sdestLat, sdestLong 으로 6개가 필요
            try {
                ...
                OutputStreamWriter osw = new OutputStreamWriter(conn.getOutputStream());
                //URL연결, 출력스트림 초기화
                sendMsg = "cno=" + strings[0] + "&locLat=" + strings[1] + "&locLong=" + strings[2] + "&type=" + "passenger";
                osw.write(sendMsg);
                osw.flush();
                if (conn.getResponseCode() == conn.HTTP_OK) {
                    InputStreamReader tmp = new InputStreamReader(conn.getInputStream(), "EUC-KR");
